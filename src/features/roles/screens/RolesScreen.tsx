      ...
      >
        ...
        {loading ? (
          ...
          <View style={styles.grid}>
            {roles.map((role) => {
              const meta = ROLE_META[role.code] ?? { icon: "shield-outline" as const, color: COLORS.primary };
              return (
                <View key={role.id} style={styles.card}>
                  ...
                  <View style={styles.levelRow}>
                    ...
                    <Text style={styles.levelText}>
                      {ACCESS_LABELS[role.niveau_acces] ?? `Niveau ${role.niveau_acces}`}
                    </Text>
                  </View>
                  <View style={styles.levelBar}>
                    <View
                      style={[
                        styles.levelFill,
                        {
                          width: `${role.niveau_acces}%` as any,
...
const styles = StyleSheet.create({
  ...
  roleName: { color: COLORS.text, fontSize: SIZES.fontBase, fontWeight: "700", marginBottom: 2 },
  roleCode: { color: COLORS.textSecondary, fontSize: SIZES.fontXs, marginBottom: SIZES.sm },
  divider: { height: 1, backgroundColor: COLORS.inputBorder, marginVertical: SIZES.sm },
  levelRow: { flexDirection: "row", alignItems: "center", gap: 4, marginBottom: SIZES.sm },
  levelText: { color: COLORS.textSecondary, fontSize: SIZES.fontXs },
  levelBar: {
    height: 4, borderRadius: 2,
    backgroundColor: COLORS.inputBorder, overflow: "hidden",
  },
